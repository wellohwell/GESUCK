  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-4 p-4">
        ...
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-zinc-100 dark:bg-zinc-800 p-4 rounded-xl animate-pulse h-20" />
        ))}
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      {activities.length === 0 ? (
        ...
        activities.map(activity => (
          <div key={activity.id} className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col gap-2 hover:shadow-md transition-shadow">
