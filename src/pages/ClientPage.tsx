...
export default function ClientPage() {
  ...
  const getStatusBadge = (status: string, isCompact = false) => {
    const styles: Record<string, string> = {
      survey: "bg-amber-500/10 text-amber-600 border-amber-500/20",
      acc: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20",
      pending: "bg-orange-500/10 text-orange-600 border-orange-500/20",
      reject: "bg-red-500/10 text-red-600 border-red-500/20",
      pending_gudang: "bg-sky-500/10 text-sky-600 border-sky-500/20",
      terkirim: "bg-zinc-500/10 text-zinc-600 border-zinc-500/20"
    };

    if (isCompact) {
      return (
        <span className={cn("text-[8px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-tighter border", styles[status] || "bg-zinc-100 text-zinc-500")}>
          {status.replace('_', ' ')}
        </span>
      );
    }
    ...
    return (
      <span className={cn("text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest border", styles[status] || "bg-zinc-100 text-zinc-500")}>
        {status.replace('_', ' ')}
      </span>
    );
  };
        ...
        <div className="max-w-xl mx-auto px-4">
          ...
          <div className="relative mb-4">
              ...
              <input 
                ...
                className="w-full pl-10 pr-10 py-3 bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-white/5 shadow-sm rounded-2xl text-[11px] font-bold outline-none focus:ring-1 focus:ring-primary transition-all"
              />
          </div>

          {/* TAB SYSTEM */}
          <div className="flex bg-zinc-100 dark:bg-zinc-900/50 p-1 rounded-2xl mb-6 shadow-inner ring-1 ring-zinc-200/50 dark:ring-white/5">
            {[
          ...
          {selectedClient && (
            <>
              ...
              >
                  ...
                  <div className="p-6 pt-2 space-y-8">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between border-b border-zinc-100 dark:border-white/5 pb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Order Aktif</h3>
                        </div>
                      ...
                      {activeOrders.length === 0 ? (
                        <div className="py-4 text-center">
                          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest italic opacity-60">Tidak ada pengiriman aktif</p>
                        </div>
                      ) : (
                              ...
                              <div key={order.id} className="py-3 first:pt-0 last:pb-0">
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-lg bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-white/5 flex items-center justify-center text-zinc-400 group-hover:text-primary transition-colors">
                                ...
                                <AnimatePresence>
                                  {isExpanded && (
                                    ...
                                    >
                                      <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-dashed border-zinc-100 dark:border-white/5">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 border-b border-zinc-100 dark:border-white/5 pb-2">
                        ...
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Riwayat Selesai</h3>
                      </div>

                      {historyOrders.length === 0 ? (
                        <div className="py-4 text-center">
                          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest italic opacity-60">Belum ada riwayat lunas</p>
                        </div>
                      ) : (
                        <div className="space-y-3 opacity-80 grayscale-[0.5]">
                          {historyOrders.map(order => {
                            const estimasiLunas = calculateEstimasiLunas(order.updatedAt || order.createdAt, order.tenor || 30, order.tenorType || 'hari');
                            return (
                              <div key={order.id} className="flex items-center justify-between py-2 border-b border-zinc-50 dark:border-white/[0.02] last:border-0 grow">
                                <div className="flex items-center gap-3">
                ...
                {canAction() && (
                  <div className="flex-shrink-0 bg-white dark:bg-zinc-950 px-6 py-4 border-t border-zinc-100 dark:border-white/5 safe-area-pb">
                ...
                {!canAction() && (
                  <div className="flex-shrink-0 p-4 pt-0 text-center safe-area-pb">
